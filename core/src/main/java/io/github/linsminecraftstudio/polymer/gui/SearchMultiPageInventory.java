...
class SearchMultiPageInventory<T> extends MultiPageInventoryHandler<T> {
    ...
    public void openInventory(Player p, int page) {
        ...
        if (inventories.isEmpty()) {
            TempPolymer.getInstance().getMessageHandler().sendTitle(p, "GUI.SearchNoResult");
            TempPolymer.getInstance().getMessageHandler().sendSubTitle(p, "GUI.SearchNoResultSub");

            TempPolymer.getInstance().getScheduler().scheduleDelay(() -> instance.openInventory(p), 5 * 20L);
        }else {
    ...
    public Component title(Player p) {
        return TempPolymer.getInstance().getMessageHandler().getColored(p, "GUI.SearchResultTitle");
    }
